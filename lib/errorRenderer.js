...
var ErrorRenderer = function() {
	...
	this.render = function(data) {
		...
		if (data.trace) {
			...
			lastTraceItem = data.trace.shift();
		}
		...
		if (lastTraceItem && lastTraceItem.file) {
			html += lastTraceItem.file;

			if (lastTraceItem.line) {
				html += ', line <span style="font-weight: bold; color: ' + this.hilightColor + ';">'
					+ lastTraceItem.line
					+ '</span>';
			}

			if (lastTraceItem.column) {
				html += ', column <span style="font-weight: bold; color: ' + this.hilightColor + ';">'
					+ lastTraceItem.column
					+ '</span>';
			}
		} else {
			html += '[unknown location]';
		}
		...
		if(data.trace) {
			...
			data.trace.forEach(function(item) {
				...
				html += '<div style="margin: 10px 0;">'
					+ '<span style="display: inline-block; position: relative; top: -2px; min-width: 20px; color: #b0b0b0; font-size: 11px; font-family: consolas, "courier new", monospace;">'
					+ n
					+ '</span> '
